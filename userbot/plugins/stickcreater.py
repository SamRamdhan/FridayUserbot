...
async def sticklet(event):
    ...
    await event.client.send_file(event.chat_id, image_stream, caption="Sam Ramadhan", reply_to=event.message.reply_to_msg_id)
    # cleanup
    try:
        os.remove(FONT_FILE)
    except:
...
@borg.on(admin_cmd(pattern="sticklet ?(?:(.*?) \| )?(.*)", allow_sudo=True))
async def sticklet(event):
    R = random.randint(0,256)
    G = random.randint(0,256)
    B = random.randint(0,256)
